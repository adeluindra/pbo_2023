...
class Mahasiswa:
    ...
    def simpan(self):
        ...
        sql="INSERT INTO mahasiswa (nim, nama, jk, prodi) VALUES " + str(val)
    ...
    def updateByNIM(self, nim):
        self.conn = mydb()
        val = (self.__nama, self.__jk, self.__prodi, nim)
        sql="UPDATE mahasiswa SET nama = %s, jk=%s, prodi=%s WHERE nim=%s"
        self.affected = self.conn.update(sql, val)
        self.conn.disconnect
        return self.affected        
